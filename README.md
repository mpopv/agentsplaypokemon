...
The game container also reads the six party slots from Pokémon Red memory four times per second. It sends a text message on the same game stream only when the party data changes. The decoder checks the exact English Pokémon Red ROM hash and validates each species, level, HP value, experience value, and party terminator. During a battle, it uses the active battle data for current HP and status. The spectator page shows the nickname, species, level, HP, status, and experience progress for each held Pokémon. The page gets original Red and Blue sprite images from the [PokeAPI sprite repository](https://github.com/PokeAPI/sprites). No sprite file is stored in this repository.
